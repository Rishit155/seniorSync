...
class Metrics extends React.Component {
  constructor(props) {
    ...
    this.bloodSugarChartRef = React.createRef();
    this.bloodPressureChartRef = React.createRef();
  ...
  fetchBloodSugarData() {
    // Example API call or database query to fetch blood sugar data
    // Replace this with your actual data fetching mechanism
    // For demonstration purposes, I'm setting dummy data
    const dummyBloodSugarData = [
      { date: '2023-01-01', level: 110 },
      { date: '2023-01-02', level: 105 },
      { date: '2023-01-03', level: 100 },
      { date: '2023-01-04', level: 95 },
      { date: '2023-01-05', level: 105 },
      { date: '2023-01-06', level: 108 },
      { date: '2023-01-07', level: 115 },
      { date: '2023-01-08', level: 120 },
      { date: '2023-01-09', level: 118 },
      { date: '2023-01-10', level: 125 },
      { date: '2023-01-11', level: 130 },
      { date: '2023-01-12', level: 128 },
      // More data for the last month...
    ];

    // Update state with fetched blood sugar data
    this.setState({ bloodSugarData: dummyBloodSugarData }, () => {
      // After state is updated, draw the blood sugar graph
      this.drawBloodSugarChart();
    });
  }

  // Add this inside the Metrics class

  fetchBloodPressureData() {
    // Example API call or database query to fetch blood pressure data
    // Replace this with your actual data fetching mechanism
    // For demonstration purposes, I'm setting dummy data
    const dummyBloodPressureData = [
      { date: '2023-01-01', pressure: '120/80' },
      { date: '2023-01-02', pressure: '122/78' },
      { date: '2023-01-03', pressure: '118/76' },
      { date: '2023-01-01', pressure: '120/80' },
      { date: '2023-01-02', pressure: '122/78' },
      { date: '2023-01-03', pressure: '118/76' },
      { date: '2023-01-01', pressure: '120/80' },
    ...
    // Update state with fetched blood pressure data
    this.setState({ bloodPressureData: dummyBloodPressureData }, () => {
      // After state is updated, draw the blood pressure chart
...
drawBloodPressureChart() {
  const { bloodPressureData } = this.state;
  const bloodPressureChartRef = this.bloodPressureChartRef.current.getContext('2d');
  ...
  // Extract systolic and diastolic values from the pressure string
  const pressures = bloodPressureData.map(data => {
    const [systolic, diastolic] = data.pressure.split('/');
    return { date: data.date, systolic, diastolic };
  });

  this.bloodPressureChart = new Chart(bloodPressureChartRef, {
    type: 'line',
    data: {
      labels: pressures.map(data => data.date),
      datasets: [{
        label: 'Systolic Pressure',
        data: pressures.map(data => data.systolic),
      ...
      },
      {
        ...
        data: pressures.map(data => data.diastolic),
    ...
    options: {
      plugins: {
        title: {
          display: true,
          text: 'Jane\'s Blood Pressure Over the Last Month',
          font: {
            size: 16
          }
        },
  ...
  drawBloodSugarChart() {
    const { bloodSugarData } = this.state;
    const bloodSugarChartRef = this.bloodSugarChartRef.current.getContext('2d');

    // Destroy existing chart if it exists
    if (this.bloodSugarChart) {
      this.bloodSugarChart.destroy();
    }

    this.bloodSugarChart = new Chart(bloodSugarChartRef, {
      type: 'line',
      data: {
        labels: bloodSugarData.map(data => data.date),
        datasets: [{
          label: 'Blood Sugar Level',
          data: bloodSugarData.map(data => data.level),
          backgroundColor: 'rgba(255, 99, 132, 0.2)',
          borderColor: 'rgba(255, 99, 132, 1)',
          borderWidth: 2,
          pointBackgroundColor: 'rgba(255, 99, 132, 1)',
      ...
      options: {
        plugins: {
          title: {
            display: true,
            text: 'Jane\'s Blood Sugar Levels Over the Last Month',
            font: {
              size: 16
            }
          },
        ...
        <div style={styles.container}>
          <div style={styles.metricsContainer}>
            ...
            <div style={styles.buttonsContainer}>
              <button style={styles.button}>Call Patient</button>
              <button style={styles.button}>Call Authorized User</button>
            </div>
          </div>
          ...
          <div style={styles.chartContainer}>
           <canvas ref={this.bloodSugarChartRef}></canvas>
          </div>
          <div style={styles.chartContainer}>
            <canvas ref={this.bloodPressureChartRef}></canvas>
          </div>
        </div>
      </div>
    );
